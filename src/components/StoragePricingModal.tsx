...
export const StoragePricingModal: React.FC = () => {
  ...
  const isMainnetOnly = selectedPlan.id !== 'plan_80gb' && selectedPlan.id !== 'plan_20gb';
  ...
  const handlePurchase = async () => {
    ...
    if (isMainnetOnly) {
      setError('This tier requires real Midnight Mainnet NIGHT tokens. On Preprod Testnet, only the 80 GB Testnet Shard is unlockable.');
      return;
    }
                ...
                return (
                  <div className={`grid grid-cols-1 ${displayedPlans.length <= 3 ? 'md:grid-cols-3' : 'md:grid-cols-4'} gap-4`}>
                    {displayedPlans.map((plan) => {
                      const isTestnetPlan = plan.id === 'plan_80gb' || plan.id === 'plan_20gb';
                      const isAlreadyClaimedBonus = isTestnetPlan && session.bonusClaimed;
                  ...
                  return (
                      ...
                      className={`relative rounded-2xl p-5 transition-all flex flex-col justify-between border ${
                        isAlreadyClaimedBonus
                          ...
                          ? isTestnetPlan
                            ...
                            : 'border-purple-500 bg-purple-50/50 shadow-lg ring-2 ring-purple-200 cursor-pointer'
                    ...
                    >
                      {isAlreadyClaimedBonus ? (
                        ...
                        <div className="absolute -top-2.5 right-4 px-2 py-0.5 rounded-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-[9px] shadow-sm uppercase">
                          MAINNET ONLY
                        </div>
                      )}
                      ...
                      <div>
                        <div className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center justify-between">
                          <span>{plan.name}</span>
                          {!isTestnetPlan && (
                            <span className="text-[9px] text-purple-700 bg-purple-100 font-bold px-1.5 py-0.2 rounded-md">Mainnet</span>
                          )}
                        </div>
                        ...
                        <div className="my-3 p-3 rounded-xl bg-white border border-slate-200">
                          ...
                          <div className="text-[10px] text-slate-400 flex items-center justify-between mt-0.5">
                            <span>/{billing === 'lifetime' ? 'forever' : billing}</span>
                            {!isTestnetPlan && (
                              <span className="text-[9px] font-bold text-purple-600 uppercase">Real NIGHT</span>
                            )}
                          </div>
                        </div>
                        ...
                        <ul className="space-y-1.5 text-xs text-slate-600">
                          {plan.features.map((f, idx) => (
                            <li key={idx} className="flex items-center space-x-1.5">
                              <Check className={`w-3.5 h-3.5 flex-shrink-0 ${isTestnetPlan ? 'text-sky-500' : 'text-purple-500'}`} />
              ...
              <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                  <div className="text-xs text-slate-500">
                    ...
                    {!isBalanceSufficient && !isMainnetOnly && (
                  ...
                  <button
                    ...
                    disabled={isProcessing || isMainnetOnly || !isBalanceSufficient}
                    className={`w-full sm:w-auto px-6 py-3 rounded-xl text-white font-bold text-xs shadow-md transition-all flex items-center justify-center space-x-2 ${
                      isMainnetOnly
                        ? 'bg-slate-400 cursor-not-allowed opacity-85'
                        : isProcessing || !isBalanceSufficient
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <span>Requires Midnight Mainnet (Real NIGHT)</span>
                    ) : (
                      <>
